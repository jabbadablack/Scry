...
inline size_t ArenaGetUsedMemory(const Arena* arena)      { assert(arena); return arena->offset; }
inline size_t ArenaGetTotalSize(const Arena* arena)        { assert(arena); return arena->total_size; }
inline size_t ArenaGetRemainingMemory(const Arena* arena)  { assert(arena); return arena->total_size - arena->offset; }
...
inline size_t PoolGetCapacity(const PoolAllocator* pool)    { assert(pool); return pool->capacity; }
inline size_t PoolGetActiveCount(const PoolAllocator* pool) { assert(pool); return pool->active_count; }

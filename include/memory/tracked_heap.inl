...
namespace engine {
    ...
    ENGINE_INLINE void TrackedHeap::AssertNoLeaks() noexcept {
        ...
        ENGINE_ASSERT(s_total_allocated.load(std::memory_order_relaxed) ==
                      s_total_freed.load(std::memory_order_relaxed),
    ...
    ENGINE_INLINE void TrackedHeap::Deallocate(void* ptr, size_t size) noexcept {
        ...
        ENGINE_ASSERT(s_current_usage.load(std::memory_order_relaxed) >= actual_size, "TrackedHeap metric underflow detected!");

...
// ── DiligentCore objects ──────────────────────────────────────────────────────
static RefCntAutoPtr<IPipelineState>         g_pPSO;
static RefCntAutoPtr<IShaderResourceBinding> g_pSRB;
static RefCntAutoPtr<IBuffer>                g_pMatrixBuffer;  // USAGE_DYNAMIC, SSBO, 64-byte stride
static RefCntAutoPtr<IBuffer>                g_pDrawParamsCB;  // USAGE_DYNAMIC, cbuffer, 64 bytes
...
// ── Per-frame CPU scratch (no heap allocation per frame) ──────────────────────
...
static char* DiscoverShaderSource(uint32_t* out_len) {
    ...
    EngineLog("[Renderer] FATAL: mesh.hlsl not found in any search path");
    return nullptr;
}
...
static bool CreatePSO() {
    ...
    // All variables MUTABLE: set once per SRB
    ...
    // No vertex input: all geometry comes through StructuredBuffer via SV_VertexID
...
void Init(ecs_world_t* world) {
    ...
    // Matrix SSBO: 64-byte stride, per-frame writable
    {
        BufferDesc bd;
        bd.Name              = "MatrixSSBO";
        ...
        bd.ElementByteStride = 64u;  // sizeof(ScryMat4) = 4 × float4
    ...
    // DrawParams cbuffer: 64 bytes (4 × float4 = one column-major viewProj)
    {
        BufferDesc bd;
    ...
    // ── Render System (Phase_React) ───────────────────────────────────────────
    {
        ...
        s.callback = [](ecs_iter_t* it) {
            ...
            // ── Collect visible entities → packed mat4 array ──────────────
            s_entity_count = 0;
            if (g_render_query) {
            ...
            // ── Upload viewProj as 4 column-vectors into cbuffer ──────────
            if (g_camera_query) {
            ...
            // ── Ensure SRB is bound to this mesh's VB SRV ─────────────────
            EnsureSRB(mesh_slot);

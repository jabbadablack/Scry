...
ScryLODGroup ScryGraphics_LoadMesh(const char* filepath) {
    ...
    const uint32_t vb0_bytes = hdr->lod0_vertex_count * 16u;
    const uint32_t ib0_bytes = hdr->lod0_index_count * 4u;
    const uint32_t vb1_bytes = hdr->lod1_vertex_count * 16u;
    const uint32_t ib1_bytes = hdr->lod1_index_count * 4u;
    const uint32_t vb2_bytes = hdr->lod2_vertex_count * 16u;
    ...
    if (g_VertexOffset * 16u + vb_total > GLOBAL_VB_SIZE ||
    ...
    const uint32_t fi0 = g_IndexOffset;
    {
        ...
        g_pContext->CopyBuffer(pStagingV, 0, RESOURCE_STATE_TRANSITION_MODE_TRANSITION,
            g_GlobalVertexBuffer, bv0 * 16u, vb0_bytes, RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
    }
    {
    ...
    const uint32_t fi1 = g_IndexOffset;
    {
        ...
        g_pContext->CopyBuffer(pStagingV, 0, RESOURCE_STATE_TRANSITION_MODE_TRANSITION,
            g_GlobalVertexBuffer, bv1 * 16u, vb1_bytes, RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
    }
    {
        RefCntAutoPtr<IBuffer> pStagingI;
    ...
    const uint32_t fi2 = g_IndexOffset;
    {
        ...
        g_pContext->CopyBuffer(pStagingV, 0, RESOURCE_STATE_TRANSITION_MODE_TRANSITION,
            g_GlobalVertexBuffer, bv2 * 16u, vb2_bytes, RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
    }
    {
        RefCntAutoPtr<IBuffer> pStagingI;
